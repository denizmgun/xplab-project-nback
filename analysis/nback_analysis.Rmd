...
```
We can't always tell beforehand which value is chosen as intercept. Therefore this has to be done again after all the data has been collected. But in this case "target" seems to be our intercept.
...
Now, we can't show that the accuracies for lures at n-1 and n+1 are 'significantly similar'. What we can try to show, at least, is that one is not significantly higher than the other. It is to be kept in mind, however, that this is not an appropriate procedure to conduct hypotheses testing but merely provides an intimation of the relationship between the two kinds of lures. The results will be stored in the column "H1c" below.

```{r}

# Testing wether lures at n+1 are significantly more accurate than lures at n-1
p.H1c = mean(post.samples$b_stimulus_typelureMnP1 > post.samples$b_stimulus_typelureMnM1)
#Considering the interaction with and the intercept for n=4
p.H1c.n4 = mean(post.samples$b_stimulus_typelureMnP1 +
                  post.samples$"b_stimulus_typelureMnP1:n4"
